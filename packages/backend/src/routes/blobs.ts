...
router.get('/:txHash/status', async (req: Request, res: Response) => {
  try {
    ...
    if (!job) {
      // Not in local DB — check on-chain via BlobKit
      const blobkit = await getBlobKit();
      try {
        const result = await blobkit.readBlob(txHash);
        return res.json({
    ...
    // If still pending in DB, re-check via getJobStatus
...
router.post('/refund', async (req: Request, res: Response) => {
  try {
    ...
    const db = getDb();
    db.prepare(`
      UPDATE blob_jobs SET status = 'refunded' WHERE job_id = ?
    `).run(jobId);
...
router.get('/gas', async (_req: Request, res: Response) => {
  try {
    const blobkit = await getBlobKit();
    // Estimate with 1KB sample to get current gas price
    ...
    return res.json({
      ...
      estimatedCostFor124KB: estimate.totalETH,
    });
